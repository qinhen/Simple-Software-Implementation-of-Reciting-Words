...
{
	bool flag = false;
    flag = m_configuration.load("conf");
    ...
    flag = m_library.link("bin/data0");///加载  !!
	if (!flag)
	{
		QMessageBox::critical(0, "Application Error!",
			QString("An internal error occurred!\n") +
            "Can't open \"bin/data0\"\n" +
			"Please contact our e-mail at xxxxxxxx and report\n" +
...
void MainWindow::setup_comp()
{
    ...
    m_mainWindow_title = new QLabel("learnEn (dev-V0.6)", this);
	...
	m_mainMenu_mainStr = new QLabel(QString::fromLocal8Bit("首页"), m_mainWindow_mainMenu);
	m_mainMenu_taskStr = new QLabel(QString::fromLocal8Bit("单词"), m_mainWindow_mainMenu);
	m_mainMenu_dictStr = new QLabel(QString::fromLocal8Bit("字典"), m_mainWindow_mainMenu);
	m_mainMenu_optiStr = new QLabel(QString::fromLocal8Bit("设定"), m_mainWindow_mainMenu);
